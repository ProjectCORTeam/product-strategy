...
| **Risk Management** | Detección de riesgo con narración y acciones de MAIA, hoy a nivel proyecto | **"Banners de riesgo de MAIA" + "Chat de MAIA"** (requiere las dos) | **La misma que MAIA** (128 companies) | PM, Director y C-Level. **Colaborador, Freelancer y Cliente no ven banners** | **Beta** desde may-26, a nivel proyecto | Baseline: **penetración 7,3%** jul-26 `[provisorio]` |
| _(fila libre para la próxima feature)_ | | | | | | |

> ✅ **Risk Management resuelto (2026-08-18): pilar de MAIA con activación separada, no feature hermana.** Tiene **Feature Access propio** ("Banners de riesgo de MAIA"), pero **exige además el de "Chat de MAIA"** y **está habilitado en toda la base de MAIA** _(confirmado con el owner)_. FA propio, **denominador idéntico**. Por el criterio de pertenencia de arriba eso lo deja **adentro de MAIA**: se mide en `06-kpi-tree` dentro de la sección de MAIA, **con corte propio por origen** (`AI_CHAT_OPEN` con `option = risk_banner`), y no como fila con denominador independiente — que es el caso de Marketplace.
>
